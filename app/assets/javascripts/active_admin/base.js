...
;(function (global, factory) {
  ...
  var onDOMReady = function onDOMReady () {
    ...
    $('.batch_actions_selector li a').on('confirm:complete', function (
      event,
      inputs
    ) {
      var val
    ...
    if (
      $('.batch_actions_selector').length &&
      $(':checkbox.toggle_all').length
    ) {
      ...
      $(document).on('change', '.paginated_collection :checkbox', function () {
        if (
          $('.paginated_collection :checkbox:checked').length &&
          $('.dropdown_menu_list').children().length
        ) {
  ...
  ;(function ($) {
    $(document).on(
      'focus',
      'input.datepicker:not(.hasDatepicker)',
      function () {
        var input = $(this)
  ...
  var DropdownMenu = (function () {
    ...
    _proto._position = function _position () {
      this.$menuList.css(
        'top',
        this.$menuButton.position().top + this.$menuButton.outerHeight() + 10
      )
  ...
  $(document)
    .ready(onDOMReady$1)
    .on('page:load turbolinks:load', onDOMReady$1)
  ...
  var Filters = (function () {
    ...
    Filters._disableEmptyInputFields = function _disableEmptyInputFields (
      event
    ) {
      var params = $(this)
        .find(':input')
  ...
  $(function () {
    ...
    $(document).on('click', 'a.button.has_many_add', function (event) {
      ...
      if (!before_add.isDefaultPrevented()) {
        var index =
          parent.data('has_many_index') ||
          parent.children('fieldset').length - 1
        parent.data({
    ...
    $(document).on(
      'change',
      '.has_many_container[data-sortable] :input[name$="[_destroy]"]',
      function () {
  ...
  var recompute_positions = function recompute_positions (parent) {
    ...
    parent.children('fieldset').each(function () {
      var destroy_input = $(this).find(
        "> ol > .input > :input[name$='[_destroy]']"
      )
      var sortable_input = $(this).find(
        "> ol > .input > :input[name$='[" + input_name + "]']"
      )
  ...
  var TableCheckboxToggler = (function (_CheckboxToggler) {
    ...
    _proto._didChangeToggleAllCheckbox = function _didChangeToggleAllCheckbox () {
      this.$container
        ...
        .toggleClass(
          'selected',
          _CheckboxToggler.prototype._didChangeToggleAllCheckbox.apply(
            this,
            arguments
          )
        )
  ...
  $(document)
    .ready(onDOMReady$2)
    .on('page:load turbolinks:load', onDOMReady$2)
  function modal_dialog (message, inputs, callback) {
    console.warn(
      'ActiveAdmin.modal_dialog is deprecated in favor of ActiveAdmin.ModalDialog, please update usage.'
    )
